...
package boomvec.common.constants
...
trait ScalarOpConstants
{
  ...
  val RT_X     = 2.U(3.W) // not-a-register (but shouldn't get a busy-bit, etc.)
                             // TODO rename RT_NAR
  val RT_VEC   = 4.U(3.W) // vector register
...
//val uopSRET = 40.U(UOPC_SZ.W)
  ...
  def NullMicroOp()(implicit p: Parameters): boomvec.common.MicroOp = {
    val uop = Wire(new boomvec.common.MicroOp)
    ...
    val cs = Wire(new boomvec.common.CtrlSignals())
...
trait RISCVConstants
{
  ...
  def GetCfiType(inst: UInt)(implicit p: Parameters): UInt = {
    val bdecode = Module(new boomvec.exu.BranchDecode)
