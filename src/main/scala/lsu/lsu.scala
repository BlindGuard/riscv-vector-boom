...
import boomvec.exu.{BrUpdateInfo, Exception, FuncUnitResp, CommitSignals, ExeUnitResp}
import boomvec.util.{BoolToChar, AgePriorityEncoder, IsKilledByBranch, GetNewBrMask, WrapInc, IsOlder, UpdateBrMask}
...
class LSUExeIO(implicit p: Parameters) extends BoomBundle()(p)
{
  ...
  val iresp    = new DecoupledIO(new boomvec.exu.ExeUnitResp(xLen))
  val fresp    = new DecoupledIO(new boomvec.exu.ExeUnitResp(xLen+1)) // TODO: Should this be fLen?
  val vresp    = new DecoupledIO(new boomvec.exu.ExeUnitResp(vLen))
}
