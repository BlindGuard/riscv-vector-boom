...
object UpdateBrMask
{
  ...
  def apply[T <: boomvec.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: T): T = {
    val out = WireInit(bundle)
    out.uop.br_mask := GetNewBrMask(brupdate, bundle.uop.br_mask)
    out
  }
  def apply[T <: boomvec.common.HasBoomUOP](brupdate: BrUpdateInfo, bundle: Valid[T]): Valid[T] = {
    val out = WireInit(bundle)
...
object ImmGen
{
  import boomvec.common.{LONGEST_IMM_SZ, IS_B, IS_I, IS_J, IS_S, IS_U}
 ...
 */
class BranchKillableQueue[T <: boomvec.common.HasBoomUOP](gen: T, entries: Int, flush_fn: boomvec.common.MicroOp => Bool = u => true.B, flow: Boolean = true)
  (implicit p: org.chipsalliance.cde.config.Parameters)
  extends boomvec.common.BoomModule()(p)
  with boomvec.common.HasBoomCoreParameters
{
  val io = IO(new Bundle {
