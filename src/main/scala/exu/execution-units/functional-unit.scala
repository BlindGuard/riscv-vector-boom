...
class ALUUnit(isJmpUnit: Boolean = false, numStages: Int = 1, dataWidth: Int)(implicit p: Parameters)
  extends PipelinedFunctionalUnit(
    ...
    dataWidth = dataWidth,
    isJmpUnit = isJmpUnit)
  with boomvec.ifu.HasBoomFrontendParameters
{
    ...
    dataWidth = dataWidth)
{
  ...
  io.resp.bits.fflags.valid       := vpu.io.resp.bits.fflags.valid
  io.resp.bits.fflags.bits.uop    := io.resp.bits.uop
  io.resp.bits.fflags.bits.flags  := vpu.io.resp.bits.fflags.bits.flags
}