...
class VpPipeline(implicit p: Parameters) extends BoomModule with HasVPUParameters
{
  ...
  require(rd_idx == numVrfLogicalReadPorts)
  ...
  for (unit <- exe_units) {
    ...
    // until does not include upper bound
    // 0 1 2 
    for (i <- 0 until 3) {
      unit.io_rrd_frf_resps(i) := vregfile.io.rrd_read_resps(rd_idx)
      rd_idx += 1
    }
  }
  require(rd_idx == numVrfLogicalReadPorts)
  ...
  vregfile.io.write_ports(0).valid := ll_wbarb.io.out.valid && ll_wbarb.io.out.bits.uop.dst_rtype === RT_FLT
  vregfile.io.write_ports(0).bits.addr := ll_wbarb.io.out.bits.uop.pdst
  vregfile.io.write_ports(0).bits.data := ll_wbarb.io.out.bits.data
